...
1. Create an account using a password of 71 characters.
2. Attempt to log in with that password plus an extra character:
  1. If it succeeds, that character is part of the PEPPER.
  2. If it fails, try a different character until it works.
3. Once logged in, change the current password from 71 characters to 70 characters.
4. Repeat the login attempts using the previous password + known PEPPER characters + a new character.
5. Continue this process until all characters of the PEPPER are discovered.
...
{{< audio src="/oposec_2025/boot.mp3" class="post-audio" >}}
